""" Parts of the U-Net model """

import torch
import torch.nn as nn
import torch.nn.functional as F
from backbone.resnet import resnet50
from backbone.EfficientNet import EfficientNet_B4

class DoubleConv(nn.Module):
    """(convolution => [BN] => ReLU) * 2"""

    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.double_conv = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True),
            nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True)
        )

    def forward(self, x):
        return self.double_conv(x)

class Down(nn.Module):
    """Downscaling with maxpool then double conv"""

    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.maxpool_conv = nn.Sequential(
            nn.MaxPool2d(2),
            DoubleConv(in_channels, out_channels)
        )

    def forward(self, x):
        return self.maxpool_conv(x)

class Up(nn.Module):
    """Upscaling then double conv"""

    def __init__(self, in_channels, out_channels, bilinear=True):
        super().__init__()

        # if bilinear, use the normal convolutions to reduce the number of channels
        if bilinear:
            self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        else:
            self.up = nn.ConvTranspose2d(in_channels // 2, in_channels // 2, kernel_size=2, stride=2)

        self.conv = DoubleConv(in_channels, out_channels)

    def forward(self, x1, x2):
        x1 = self.up(x1)
        # input is CHW
        diffY = x2.size()[2] - x1.size()[2]
        diffX = x2.size()[3] - x1.size()[3]

        x1 = F.pad(x1, [diffX // 2, diffX - diffX // 2,
                        diffY // 2, diffY - diffY // 2])
        # if you have padding issues, see
        # https://github.com/HaiyongJiang/U-Net-Pytorch-Unstructured-Buggy/commit/0e854509c2cea854e247a9c615f175f76fbb2e3a
        # https://github.com/xiaopeng-liao/Pytorch-UNet/commit/8ebac70e633bac59fc22bb5195e513d5832fb3bd
        x = torch.cat([x2, x1], dim=1)
        return self.conv(x)

class OutConv(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(OutConv, self).__init__()
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=1)
        self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)

    def forward(self, x):
        x = self.up(x)
        return self.conv(x)


class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True,backbone= 'EfficientNet_B4', pretrained_base =False):
        super(UNet, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear

        models = {}
        models['resnet18'] = "/home/zf/.torch/models/resnet18-5c106cde.pth"
        models['resnet34'] = "/home/zf/.torch/models/resnet34-333f7ec4.pth"
        models['resnet50'] = "/home/zf/.torch/models/resnet50-19c8e357.pth"
        models['resnet101'] = "/home/zf/.torch/models/resnet101-5d3b4d8f.pth"
        models['vgg16'] = "/home/zf/.torch/models/vgg16-00b39a1b.pth"
        models['xception39'] = "/home/zf/.torch/models/xception-43020ad28.pth"
        models['EfficientNet_B4'] = None
        if pretrained_base:
            self.pretrained = eval(backbone)(pretrained_model=models[backbone])
        else:
            self.pretrained = eval(backbone)(pretrained_model=None)

        self.up1 = Up(1952, 256, bilinear)
        self.up2 = Up(312, 128, bilinear)
        self.up3 = Up(160, 64, bilinear)
        self.up4 = Up(88, 64, bilinear)
        self.outc = OutConv(64, n_classes)

    def forward(self, x):
        x1,x2,x3,x4,x5 =self.pretrained(x)
        x = self.up1(x5, x4)
        x = self.up2(x, x3)
        x = self.up3(x, x2)
        x = self.up4(x, x1)
        logits = self.outc(x)
        return logits


x = torch.randn(2,3,512,512)
model = UNet(n_channels=3,n_classes=5)
out = model(x)
print(out.size())
# from torchsummary import summary
# summary(model,(3,512,512))